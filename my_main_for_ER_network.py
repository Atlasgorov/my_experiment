...
def experiment_on_ER(percentage):
    G = nx.read_gexf('./graph_preprocess/ER_graph.gexf')
    print(nx.is_directed(G))
    # 我选择社区标号为30的社区为谣言社区，因为它规模相对很大，总节点数为314个,社区划分之后的总社区数量是42个。
    rumor_community_size = 314  # 谣言社区包含的节点总数
    rumor_community_label = '30'  # 选择第30号社区作为谣言社区
    ...
    # 存储不同谣言规模下的结果
    f1 = open(r'./experiment_results/ER_random_network/results_new_ER.txt', 'a')
    f1.write("******************************************************\n")
