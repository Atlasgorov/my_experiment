...
def experiment_on_Gnutella08():
    G = nx.read_gexf('./graph_preprocess/Gnutella08.gexf')
    # 我选择社区标号为7的社区为谣言社区，因为它规模最大，总节点数为582个,社区划分之后的总社区数量是26个。
    rumor_community_size = 582  # 谣言社区包含的节点总数
    rumor_community_label = '7'  # 选择第7号社区作为谣言社区
    rumor_sources_percentage = 0.015 # 谣言源节点比例设为1.5%
    rumor_sources_number = int(rumor_community_size * rumor_sources_percentage) # 8
    ...
    # 得到相应的三种算法所需封锁节点的数量和算法所需的运行时间，最后得到每个谣言源规模所需的平均封锁节点数量和平均运行时间
    ...
    while simulation_count < simulation_num:
        # 预处理步骤，得到子图G1
        rumor_originators = mf.random_chose_rumor_sources(rumor_community_nodes,rumor_sources_number)  # 从谣言社区内部随机选择节点作为谣言源节点
        A, Length_A, B0, Length_B0, M0, Length_M0, k ,time0 = mf.bfs_0(G, rumor_originators, rumor_community_label, protection_level=0.9) # 从源节点出发BFS搜索
    ...
    np.savetxt(r'./experiment_results/Gnutella08/my_final_result.csv', my_final_result, delimiter=',',header=data_header,fmt="%d")
    np.savetxt(r'./experiment_results/Gnutella08/k_core_result.csv', k_core_result, delimiter=',',header=data_header,fmt="%d")
    np.savetxt(r'./experiment_results/Gnutella08/max_degree_result.csv', max_degree_result, delimiter=',',header=data_header,fmt="%d")
    np.savetxt(r'./experiment_results/Gnutella08/betweenness_centrality_result.csv', betweenness_centrality_result, delimiter=',',header=data_header,fmt="%d")
    np.savetxt(r'./experiment_results/Gnutella08/num_of_reachable_nodes_from_sources.csv', num_of_reachable_nodes_from_sources, delimiter=',',header=data_header,fmt="%d")
    np.savetxt(r'./experiment_results/Gnutella08/num_of_common_bridges.csv', num_of_common_bridges, delimiter=',',header=data_header,fmt="%d")
    np.savetxt(r'./experiment_results/Gnutella08/num_of_special_bridges.csv', num_of_special_bridges, delimiter=',',header=data_header,fmt="%d")
    np.savetxt(r'./experiment_results/Gnutella08/run_time_my_method.csv', run_time_my_method, delimiter=',',header=data_header)
    np.savetxt(r'./experiment_results/Gnutella08/run_time_k_core.csv', run_time_k_core, delimiter=',', header=data_header)
    np.savetxt(r'./experiment_results/Gnutella08/run_time_max_degree.csv', run_time_max_degree, delimiter=',', header=data_header)
    np.savetxt(r'./experiment_results/Gnutella08/run_time_betweenness_centrality.csv', run_time_betweenness_centrality,
               delimiter=',',
               header=data_header)
    """
    # 打印平均结果
