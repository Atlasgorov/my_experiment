...
def experiment_on_Epinions():
    G = nx.read_gexf('./graph_preprocess/Epinions.gexf')
    # 我选择社区标号为'2460'的社区为谣言社区，因为它规模适中，总节点数为2835个,社区划分之后的总社区数量是6872个。
    rumor_community_size = 2835  # 谣言社区包含的节点总数
    rumor_community_label = '2460'  # 选择第2460号社区作为谣言社区
    rumor_sources_percentage = 0.015  # 谣言源节点比例设为1.5%
    rumor_sources_number = int(rumor_community_size * rumor_sources_percentage)  # 42
    ...
    num_of_common_bridges = np.concatenate(
        ([rumor_sources_number], num_of_common_bridges, [average_num_of_common_bridges]))
    num_of_special_bridges = np.concatenate(
        ([rumor_sources_number], num_of_special_bridges, [average_num_of_special_bridges]))
    ...
    np.savetxt(r'./experiment_results/Epinions/my_final_result.csv', my_final_result, delimiter=',',
               header=data_header, fmt="%d")
    np.savetxt(r'./experiment_results/Epinions/k_core_result.csv', k_core_result, delimiter=',', header=data_header,
               fmt="%d")
    ...
    np.savetxt(r'./experiment_results/Epinions/num_of_common_bridges.csv', num_of_common_bridges, delimiter=',',
               header=data_header, fmt="%d")
    np.savetxt(r'./experiment_results/Epinions/num_of_special_bridges.csv', num_of_special_bridges, delimiter=',',
               header=data_header, fmt="%d")
    np.savetxt(r'./experiment_results/Epinions/run_time_my_method.csv', run_time_my_method, delimiter=',',
               header=data_header)
    np.savetxt(r'./experiment_results/Epinions/run_time_k_core.csv', run_time_k_core, delimiter=',',
               header=data_header)
